...
// Extract per-building color from API cache:
// energy_result.end.color.energy_demand_specific_color
FString ColorHex;

TSharedPtr<FJsonObject> EnergyResultObj;
if (BuildingObject->HasField(TEXT("energy_result")))
{
    EnergyResultObj = BuildingObject->GetObjectField(TEXT("energy_result"));
}

if (!EnergyResultObj.IsValid())
{
    UE_LOG(LogTemp, Warning, TEXT("🎨 COLOR Building %s: Missing 'energy_result' - skipping color cache for this building."), *BuildingGmlId);
    continue;
}

if (!EnergyResultObj->HasField(TEXT("end")))
{
    UE_LOG(LogTemp, Warning, TEXT("🎨 COLOR Building %s: Missing 'energy_result.end' - skipping color cache for this building."), *BuildingGmlId);
    continue;
}

const TSharedPtr<FJsonObject> EndObjectLocal = EnergyResultObj->GetObjectField(TEXT("end"));
if (!EndObjectLocal.IsValid() || !EndObjectLocal->HasField(TEXT("color")))
{
    UE_LOG(LogTemp, Warning, TEXT("🎨 COLOR Building %s: Missing 'energy_result.end.color' - skipping color cache for this building."), *BuildingGmlId);
    continue;
}

const TSharedPtr<FJsonObject> EndColorObj = EndObjectLocal->GetObjectField(TEXT("color"));
if (!EndColorObj.IsValid() || !EndColorObj->HasTypedField<EJson::String>(TEXT("energy_demand_specific_color")))
{
    UE_LOG(LogTemp, Warning, TEXT("🎨 COLOR Building %s: Missing 'energy_result.end.color.energy_demand_specific_color' - skipping color cache for this building."), *BuildingGmlId);
    continue;
}

ColorHex = EndColorObj->GetStringField(TEXT("energy_demand_specific_color"));
if (!ColorHex.StartsWith(TEXT("#")))
{
...
{
	// Cesium 3D Tiles Styling (NOT Mapbox expressions).
	// We build a JSON style with "conditions" so each feature (building) can be colored
	// based on its per-feature metadata property "gml:id".
	//
	// Example:
	// {
	//   "color": {
	//     "conditions": [
	//       ["${feature['gml:id']} === 'DEBW_0010008wid9'", "color('#66b032')"],
	//       ["true", "color('#ffffff')"]
	//     ]
	//   }
	// }
	...
	if (BuildingColorCache.Num() == 0)
	{
		// default (white-ish) to make it obvious styling is active but no cache is loaded
		return TEXT("{\"color\":{\"conditions\":[[\"true\",\"color('#ffffff')\"]]}}");
	}

	FString Json;
	Json += TEXT("{\"color\":{\"conditions\":[");
	int32 Added = 0;

	// NOTE: JSON can get huge if you have many buildings. We keep a conservative cap to
	// avoid blowing past typical limits. If you need all buildings, we should switch to
	// a texture lookup or server-side styling.
	const int32 MaxRules = 2500;

	for (const auto& Pair : BuildingColorCache)
	{
		if (Added >= MaxRules) break;

		const FString& GmlId = Pair.Key;
		const FLinearColor& Color = Pair.Value;

		// Convert to sRGB hex.
		const FColor SRGB = Color.ToFColor(true);
		const FString Hex = FString::Printf(TEXT("#%02X%02X%02X"), SRGB.R, SRGB.G, SRGB.B);

		// Escape any quotes/backslashes in the id (safety).
		FString SafeId = GmlId;
		SafeId.ReplaceInline(TEXT("\"), TEXT("\\"));
		SafeId.ReplaceInline(TEXT("\""), TEXT("\\""));

		// Condition string: ${feature['gml:id']} === "ID"
		const FString Condition = FString::Printf(TEXT("${feature['gml:id']} === \"%s\""), *SafeId);

		if (Added > 0) Json += TEXT(",");
		Json += FString::Printf(TEXT("[\"%s\",\"color('%s')\"]"), *Condition, *Hex);

		Added++;
	}

	// Fallback rule (keep buildings visible even if not in cache)
	Json += TEXT(",[\"true\",\"color('#ffffff')\"]]}}");

	UE_LOG(LogTemp, Warning, TEXT("STYLE Built Cesium 3D Tiles style JSON. Rules=%d, Length=%d"), Added, Json.Len());
	return Json;
}
...
void ABuildingEnergyDisplay::ApplyCesiumTilesetStyling(AActor* CesiumActor)
{
	...
	const FString StyleJson = CreateCesiumColorExpression();
	UE_LOG(LogTemp, Warning, TEXT("STYLE Applying Cesium 3D Tiles style JSON to %s (len=%d)"), *CesiumActor->GetName(), StyleJson.Len());

	// Cesium-for-Unreal stores style in a UCesium3DTilesetStyle object (or similar).
	// We set it via reflection to avoid hard plugin dependencies in this class.
	auto TrySetStyleOnObject = [&](UObject* Obj) -> bool
	{
		if (!Obj) return false;

		// 1) If this object *is* a style object, set its Json / StyleJson / Style field.
		const FString ClassName = Obj->GetClass()->GetName();
		const bool bLooksLikeStyle = ClassName.Contains(TEXT("Cesium3DTilesetStyle")) || ClassName.Contains(TEXT("TilesetStyle"));
		...
		if (bLooksLikeStyle)
		{
			for (TFieldIterator<FProperty> It(Obj->GetClass()); It; ++It)
			{
				...
				if (!(PN.Equals(TEXT("Json"), ESearchCase::IgnoreCase) ||
					  PN.Equals(TEXT("StyleJson"), ESearchCase::IgnoreCase) ||
					  PN.Equals(TEXT("Style"), ESearchCase::IgnoreCase)))
				{
					continue;
				}
				...
				{
					SP->SetPropertyValue_InContainer(Obj, StyleJson);
					UE_LOG(LogTemp, Warning, TEXT("STYLE Set %s.%s (string)"), *ClassName, *PN);
					return true;
				}
				if (FTextProperty* TP = CastField<FTextProperty>(P))
				{
					TP->SetPropertyValue_InContainer(Obj, FText::FromString(StyleJson));
					UE_LOG(LogTemp, Warning, TEXT("STYLE Set %s.%s (text)"), *ClassName, *PN);
					return true;
				}
			}
		}

		// 2) Otherwise, look for a property named TilesetStyle/Style that points to a style object,
		// or contains a JSON field we can set.
		for (TFieldIterator<FProperty> It(Obj->GetClass()); It; ++It)
		{
			FProperty* P = *It;
			const FString PN = P->GetName();
			...
			const bool bCandidate =
				PN.Equals(TEXT("TilesetStyle"), ESearchCase::IgnoreCase) ||
				PN.Equals(TEXT("Style"), ESearchCase::IgnoreCase) ||
				PN.Contains(TEXT("TilesetStyle"), ESearchCase::IgnoreCase);

			if (!bCandidate) continue;

			// Object property: UObject* (likely the style asset/object)
			if (FObjectProperty* OP = CastField<FObjectProperty>(P))
			{
				UObject* StyleObj = OP->GetObjectPropertyValue_InContainer(Obj);

				// If null, try to instantiate if we can.
				if (!StyleObj && OP->PropertyClass)
				{
					StyleObj = NewObject<UObject>(Obj, OP->PropertyClass);
					if (StyleObj)
					{
						OP->SetObjectPropertyValue_InContainer(Obj, StyleObj);
						UE_LOG(LogTemp, Warning, TEXT("STYLE Created style object %s for %s.%s"), *StyleObj->GetClass()->GetName(), *Obj->GetClass()->GetName(), *PN);
					}
				}

				if (TrySetStyleOnObject(StyleObj))
				{
					return true;
				}
			}

			// Struct property: try to find a "Json"/"StyleJson" string field inside the struct.
			if (FStructProperty* StrP = CastField<FStructProperty>(P))
			{
				void* StructPtr = StrP->ContainerPtrToValuePtr<void>(Obj);
				if (!StructPtr) continue;

				for (TFieldIterator<FProperty> Sit(StrP->Struct); Sit; ++Sit)
				{
					FProperty* SP = *Sit;
					const FString SPN = SP->GetName();
					if (!(SPN.Equals(TEXT("Json"), ESearchCase::IgnoreCase) ||
						  SPN.Equals(TEXT("StyleJson"), ESearchCase::IgnoreCase) ||
						  SPN.Equals(TEXT("Style"), ESearchCase::IgnoreCase)))
					{
						continue;
					}

					if (FStrProperty* SSP = CastField<FStrProperty>(SP))
					{
						SSP->SetPropertyValue_InContainer(StructPtr, StyleJson);
						UE_LOG(LogTemp, Warning, TEXT("STYLE Set struct %s.%s.%s"), *Obj->GetClass()->GetName(), *PN, *SPN);
						return true;
					}
					if (FTextProperty* STP = CastField<FTextProperty>(SP))
					{
						STP->SetPropertyValue_InContainer(StructPtr, FText::FromString(StyleJson));
						UE_LOG(LogTemp, Warning, TEXT("STYLE Set struct %s.%s.%s (text)"), *Obj->GetClass()->GetName(), *PN, *SPN);
						return true;
					}
				}
			}
	...
	// Try actor itself
	bApplied |= TrySetStyleOnObject(CesiumActor);

	// Try all components
	TArray<UActorComponent*> Comps;
	CesiumActor->GetComponents(Comps);
	for (UActorComponent* C : Comps)
	{
		if (!C) continue;
		bApplied |= TrySetStyleOnObject(C);
	}

	if (!bApplied)
	{
		UE_LOG(LogTemp, Error, TEXT("STYLE Failed to locate a Cesium style property/object on %s."))
			;
		UE_LOG(LogTemp, Error, TEXT("STYLE Next step: log Cesium actor/component properties to confirm the real style field name."));
	}

	// Force render state refresh so the new style takes effect.
	CesiumActor->MarkComponentsRenderStateDirty();

#if WITH_EDITOR
	CesiumActor->Modify();
	CesiumActor->PostEditChange();
#endif
}
